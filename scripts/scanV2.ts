...
export const scanV2 = async (
  ...
  if (fleet.getCurrentSector().hasStarbase && fleet.getCurrentState().StarbaseLoadingBay && !scanSector.key.equals(backStarbaseSector.key)) {
    ...
    if (!fleet.getOnlyDataRunner()) {
      await actionWrapper(loadCargo, fleet, ResourceName.Food, CargoPodType.CargoHold, new BN(fleet.getCargoHold().maxCapacity - (fleet.getStats().miscStats.sduPerScan * 3)));
  ...
  //if (fleet.getCurrentState().Idle || fleet.getCurrentState().MoveSubwarp || fleet.getCurrentState().MoveWarp) {
    ...
    if (!fleet.getCurrentSector().key.equals(scanSector.key)) {
      ...
      if (fuelNeeded !== 0 && fleet.getFuelTank().loadedAmount.lt(new BN(fuelNeeded))) {
        return { type: "NotEnoughFuelToGo" as const };
      }

      // move to sector (->)
      if (movementGoBack === MovementType.Warp) {
        for (let i = 1; i < goRoute.length; i++) {
          const sectorTo = goRoute[i];
          const warp = await actionWrapper(warpToSector, fleet, sectorTo, fuelNeeded, i < goRoute.length - 1);
      ...
      if (movementGoBack === MovementType.Subwarp) {
        const sectorTo = goRoute[1];
        const subwarp = await actionWrapper(subwarpToSector, fleet, sectorTo, fuelNeeded);
    ...
    if (!fleetAfterScanSector) return { type: "FleetCurrentSectorError" as const };
    ...
    if (!fleetAfterScanSector.key.equals(backStarbaseSector.key)) {
      ...
      if (movementGoBack === MovementType.Warp) {
        for (let i = 1; i < backRoute.length; i++) {
          ...
          const warp = await actionWrapper(warpToSector, fleet, sectorTo, fuelNeeded, true);
          if (warp.type !== "Success") {
            return warp;
          }
        }   
